...
func (tm *TaskManager) AddTask(task *model.Task) error {
	sqlStatement := `INSERT INTO tasklist (assignee, title, deadline, done) VALUES($1, $2, $3, $4)`
	_, err := tm.db.Exec(sqlStatement, task.Assignee, task.Title, task.Deadline, task.Done)
	if err != nil {
		return err
	}
	return nil
